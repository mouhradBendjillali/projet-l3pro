...
var oscillo = [{
  type: "spline",
  showInLegend: false, //define legend text
  legendText: "",
  dataPoints: []
		}, {
  type: "spline",
    ...
    if (pin.analogChannel == 127) {
      ...
      $("#pinMode" + pinNumber).change(function () {
        ...
        $(function () {
          ...
          if ($("#pinMode" + pinNumber).val() == "IN") {
            $("#afficheValNum" + pinNumber).val("NULL");
            board.pinMode(pinNumber, board.MODES.INPUT);
            board.digitalRead(pinNumber, function (value) {
              $("#afficheValNum" + pinNumber).val(value);
            });
          }
...
function updateAna() {
  board.analogPins.map(function (pin, pinNumber) {
    ...
    $("#pinModeA" + pinNumber).change(function () {
      ...
      $("#actionAna" + pinNumber).html(pin.toHtml(ana, disabled));
      ...
      if ($("#pinModeA" + pinNumber).val() == "IN") {
        ...
        board.analogRead(pinNumber, function (value) {
          ...
          if ($("#oscilloAna" + pinNumber).is(':checked')) {
            if (oscillo[0].dataPoints.length <= dataLength) {
              testCompt[pinNumber] = 0;
              oscillo[0].dataPoints.push({
                x: xVal,
                y: value
            ...
            /*else if(oscillo[1].dataPoints.length <= dataLength){
              console.log("oscillo 1111111111111");
              testCompt[pinNumber] = 1;
              oscillo[1].dataPoints.push({
                x: xVal,
                y: value
              });
              xVal++;

              if (oscillo[1].dataPoints.length > dataLength )
              {
      ...
      $(function () {
        $("#oscilloAna" + pinNumber).change(function () {
          if ($(this).is(':checked')) {
            oscillo[0].showInLegend = true;
            oscillo[0].legendText = "A" + pinNumber;
            for (var i = 0; i < board.analogPins.length; i++) {
              if (i != pinNumber) {
                $("#oscilloAna" + i).attr("disabled", true);
              }
            }

          } else if (checked == pinNumber) {
            for (var i = 0; i < board.analogPins.length; i++) {
              if (i != pinNumber) {
                $("#oscilloAna" + i).removeAttr("disabled");
              }
            }
            oscillo[0].showInLegend = false;
            oscillo[testCompt[pinNumber]].dataPoints = [];
          }
          canvasOscillo(oscillo, pinNumber);
          //dialog error else if ( > 5lignes du Graph) 
        });
      });
...
function saveFile(fileName) {
  ...
  var blob = new Blob([xmlSave.content], {
